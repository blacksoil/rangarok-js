...
SprParser.PALFrame = function( buffer, offset, version ) {
	
	version = version || 0x200;
	...
	if( version > 0x101 ) {
		...
		this.byteLength = this.width * this.height;
	}
...
SprParser.prototype.parseFrameHeaders = function( buffer ) {
	...
	for( var i = 0; i < this.header.frameCount; i++ ) {
		...
		frame = new SprParser.PALFrame( buffer, offset, this.header.version );
		
		if( this.header.version > 0x101 ) {
			offset += 6 + frame.byteLength;
		} else {
			// version <= 0x101 doesn't have length attribute
