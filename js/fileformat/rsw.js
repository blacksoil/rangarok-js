...
function RSW( buffer ) {
	...
	this.isArcturus = Settings.isArcturus || false;
	...
	this.parseFormat = function() {
		...
		if( this.header.version.compareTo(1, 2) < 0 )
			throw 'RSW :: Unable to read file version ' + this.header.version.toString;
		...
		for( var i = 0; i < this.header.numObjects; i++ ) {
			...
			if( type == 1 ) {
				...
				if( this.isArcturus ) {
					...
					// Name[80] correct length?
					// SprName[80] correct length?
					// unknown[48]
					
					// Log NPC name
					//console.log(data.getString( offset + 80, 80 ));
			...
			} else if( this.isArcturus && type == 5 ) {
				
				// Sound source for Arcturus
				
				// name[80]
				// waveName[80] 
				
				// unknown[20] - likely float x, float y, float z, int a, int b
				
				offset += 180;
			
			} else {
				if( this.isArcturus ) {
					console.warn("RSW: Invalid object type " + type + ", stopping early!");
					break;
				}
				throw "RSW: Unknown object type " + type;
			}
